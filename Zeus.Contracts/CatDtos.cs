...
/// there is no universal TCP-CAT port; match it to your client).
/// <para><paramref name="AutoReport"/> is the TCP listener equivalent of
/// piHPSDR's AutoRprt: each newly accepted client starts in AI1 mode so
/// devices that expect unsolicited frequency updates but never send
/// <c>AI1;</c> still get them.</para></summary>
...
public sealed record CatStatus(
    ...
    bool CurrentAutoReport,
    bool PendingEnabled,
    int PendingPort,
    string PendingBindAddress,
    bool PendingAutoReport,
    int ClientCount,
    bool PortAvailable,
