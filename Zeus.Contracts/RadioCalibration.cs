...
/// the safe PA ceiling used for meter scaling. Each detector has its own bridge
/// and ADC-offset constants; the power math is otherwise the same across boards
/// — <c>watts = volts² / bridge_volt</c> where
/// <c>volts = (adc − cal_offset) / 4095 · ref_voltage</c> — only the constants
/// differ. Thetis <c>console.cs</c> <c>computeAlexFwdPower</c> and
/// <c>computeRefPower</c> are the authoritative references.
/// </summary>
    ...
    double MaxWatts)
{
    ...
    public static readonly RadioCalibration HermesLite2 = new(
        ...
        RefVoltage: 3.3,
        AdcCalOffset: 6,
        ReverseBridgeVolt: 1.5,
        ReverseAdcCalOffset: 6,
        SixMeterReverseBridgeVolt: 1.5,
        MaxWatts: 5.0);
    ...
    public static readonly RadioCalibration Hermes = new(
        ...
        RefVoltage: 3.3,
        AdcCalOffset: 6,
        ReverseBridgeVolt: 0.09,
        ReverseAdcCalOffset: 3,
        SixMeterReverseBridgeVolt: 0.09,
        MaxWatts: 10.0);

    /// <summary>
    /// ANAN-100 / 100B / 100D (board id 0x04 = Angelia) — Thetis
    /// <c>console.cs:25063-25072</c>. Bridge 0.095, ref 3.3, offset 6.
    /// Reflected power uses offset 3 and switches its bridge from 0.095 to
    /// 0.5 on 6 m.
    /// </summary>
    public static readonly RadioCalibration Anan100 = new(
        ...
        RefVoltage: 3.3,
        AdcCalOffset: 6,
        ReverseBridgeVolt: 0.095,
        ReverseAdcCalOffset: 3,
        SixMeterReverseBridgeVolt: 0.5,
        MaxWatts: 100.0);

    /// <summary>
    /// ANAN-200D (board id 0x05 = Orion) — Thetis. Forward power uses bridge
    /// 0.108, ref 5.0, offset 4; reflected power uses offset 2 and a 0.5
    /// bridge on 6 m.
    /// </summary>
    public static readonly RadioCalibration Anan200 = new(
        ...
        RefVoltage: 5.0,
        AdcCalOffset: 4,
        ReverseBridgeVolt: 0.108,
        ReverseAdcCalOffset: 2,
        SixMeterReverseBridgeVolt: 0.5,
        MaxWatts: 200.0);

    /// <summary>
    /// ANAN-7000DLE / ANAN-G1 / ANAN-G2 / ANAN-G2-1K / Anvelina Pro3 /
    /// RedPitaya — Thetis. Forward power uses bridge 0.12, ref 5.0, offset 32;
    /// reflected power uses bridge 0.15, offset 28 (bridge 0.7 on 6 m).
    /// KB2UKA's test G2 MkII reports board id 0x0A
    ...
    public static readonly RadioCalibration OrionMkII = new(
        ...
        RefVoltage: 5.0,
        AdcCalOffset: 32,
        ReverseBridgeVolt: 0.15,
        ReverseAdcCalOffset: 28,
        SixMeterReverseBridgeVolt: 0.7,
        MaxWatts: 100.0);
    ...
    public static readonly RadioCalibration OrionMkIIAnan8000 = new(
        ...
        RefVoltage: 5.0,
        AdcCalOffset: 18,
        ReverseBridgeVolt: 0.08,
        ReverseAdcCalOffset: 16,
        SixMeterReverseBridgeVolt: 0.08,
        MaxWatts: 200.0);
