...
/// </list>
///
/// This file is intentionally duplicated across <c>Zeus.Protocol1</c>,
/// <c>Zeus.Protocol2</c>, and <c>Zeus.Dsp.Wdsp</c>; the projects don't share
/// a utility assembly. Keep all three copies in sync.
/// </remarks>
// Public so Station.Engine.Hosting (TxTuneDriver pump) can promote its dedicated
...
public static partial class RealtimeThreadPriority
{
    ...
    private static void PromoteMacOs(ILogger log)
    {
        ...
        try
        {
            // mach_thread_self returns a +1 send right. Capture it once for
            // thread_policy_set and release it in the finally below.
            threadPort = mach_thread_self();
            ...
            if (tb.numer != 0 && tb.denom != 0)
            {
                ...
                int rcRt = thread_policy_set(
                    threadPort, THREAD_TIME_CONSTRAINT_POLICY,
        ...
        finally
        {
            if (threadPort != 0)
            {
                try
                {
                    int rcDeallocate = mach_port_deallocate(mach_task_self(), threadPort);
                    if (rcDeallocate != 0)
                        log.LogWarning("thread.priority mach_port_deallocate rc={Rc}", rcDeallocate);
                }
                catch (Exception ex)
                {
                    log.LogWarning(ex, "thread.priority mach_port_deallocate threw");
                }
            }
        }
