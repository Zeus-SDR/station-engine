...
public sealed class PsAutoAttenuateService : BackgroundService
{
    ...
    // HermesC10 / HermesII single-ADC P1 path — the same plain walk as the P2
    // dance (deliberately NOT the HL2 variant's -28 floor). A short, flow-
    // proven acquisition dwell escapes a first-fit stall at the protective
    // 31 dB default. Reuses the P2AutoAttState shape; the wire write
    ...
    // One second at the active 10 Hz cadence gives calcc a full fit window at
    // each intermediate attenuation before the escape takes another step.
    private const int C10CollectStallDwellTicks = 10;
    private int _c10CollectStallTicks;
...
#pragma warning restore CS0162
    ...
    // HermesC10 / HermesII single-ADC P1 dance — the plain Tick1P2 walk with the wire
    ...
    // The collect-stall escape runs before the no-new-calc gate, but only when
    // paired blocks are demonstrably advancing; all completed-fit behavior
    // and persistence policy remain unchanged.
    //
    // Once we're past Monitor we MUST complete the cycle so PS gets re-enabled —
    // the early gates at the top of Tick1 honour that by only running while
    // in Monitor.
    private void Tick1SingleAdcP1(StateDto s, IDspEngine engine, IProtocol1Client p1)
    {
        switch (_c10State)
        {
            case P2AutoAttState.Monitor:
            {
                ...
                // An unset Hermes-family register starts at the protective
                // 31 dB rail. If that starves calcc before its first fit, the
                // normal CalibrationAttemptsChanged gate can never move it.
                // Require consecutive observations of newly delivered paired
                // blocks so a missing/misrouted feedback stream never causes
                // an automatic attenuation write.
                long delivered = p1.PsFeedbackBlocksDelivered;
                bool collectStalled = psm.CalibrationAttempts == 0 && feedback <= 0;
                if (collectStalled && delivered > _c10LastFeedbackBlocks)
                {
                    _c10CollectStallTicks++;
                    _c10LastFeedbackBlocks = delivered;
                    if (_c10CollectStallTicks >= C10CollectStallDwellTicks)
                    {
                        ...
                        if (_currentAttnDb > TxAttnMinDb)
                        {
                            ...
                            engine.SetPsControl(autoCal: false, singleCal: false);
                            _log.LogInformation(
                                "psAutoAttn.c10.collect-stall blocks={Blocks} delta={Delta} attn={Db} — walking attenuation down",
                                delivered, _c10DeltaDb, _currentAttnDb);
                            _c10State = P2AutoAttState.SetNewValues;
                            return;
                        }
                        if (!_c10DeadPathWarned)
                        {
                            _c10DeadPathWarned = true;
                            _log.LogWarning(
                                "psAutoAttn.c10.stall attn=0 fb=0 — feedback path dead, check tap/cabling");
                        }
                    }
                }
                else
                {
                    _c10CollectStallTicks = 0;
                    _c10LastFeedbackBlocks = delivered;
                    if (!collectStalled) _c10DeadPathWarned = false;
                }
    ...
    private void ResetC10CollectStall()
    {
        _c10CollectStallTicks = 0;
