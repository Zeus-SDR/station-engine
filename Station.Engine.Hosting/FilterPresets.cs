...
public static class FilterPresets
{
    ...
    // DIGU/DIGL passbands are one-sided in Zeus (see SignedFilterForMode /
    // WdspDspEngine.ApplyBandpassForMode: DIGU sits in the positive baseband
    // (0..+N), DIGL in the negative (-N..0)). Thetis presets the pair symmetric
    // around 0 because Thetis re-maps by mode inside the WDSP RXA setter; Zeus
    // does not, so a symmetric ±N pair here would abs-fold to (N, N) — a
    // zero-width DIGU passband floored to a 10 Hz slot at ±N Hz (issue #159:
    // audio disappears when picking any DIGU/DIGL preset).
    private static readonly FilterPresetEntry[] Digl =
    [
        new("F1",   "3.0k", -3000,   0, false),
        new("F2",   "2.5k", -2500,   0, false),
        new("F3",   "2.0k", -2000,   0, false),
        new("F4",   "1.5k", -1500,   0, false),
        new("F5",   "1.0k", -1000,   0, false),
        new("F6",   "800",   -800,   0, false),
        new("F7",   "600",   -600,   0, false),
        new("F8",   "300",   -300,   0, false),
        new("F9",   "150",   -150,   0, false),
        new("F10",  "75",     -75,   0, false),
        new("VAR1", "Var 1",  -800,  0, true),
        new("VAR2", "Var 2",  -800,  0, true),
    ];

    private static readonly FilterPresetEntry[] Digu =
    [
        new("F1",   "3.0k",     0, 3000, false),
        new("F2",   "2.5k",     0, 2500, false),
        new("F3",   "2.0k",     0, 2000, false),
        new("F4",   "1.5k",     0, 1500, false),
        new("F5",   "1.0k",     0, 1000, false),
        new("F6",   "800",      0,  800, false),
        new("F7",   "600",      0,  600, false),
        new("F8",   "300",      0,  300, false),
        new("F9",   "150",      0,  150, false),
        new("F10",  "75",       0,   75, false),
        new("VAR1", "Var 1",    0,  800, true),
        new("VAR2", "Var 2",    0,  800, true),
    ];
