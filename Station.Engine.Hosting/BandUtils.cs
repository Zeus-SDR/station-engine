...
public static class BandUtils
{
    ...
    /// 10 MHz). Only consulted by <see cref="RadioService.RestoreBandMode"/> on
    /// FIRST visit to a band (no stored memory) and only when the live mode is
    /// LSB or USB — CW / digital / AM / FM carry over unchanged so a CW or
    /// FT8 operator crossing bands isn't smashed to voice.
    /// </summary>
    public static RxMode DefaultSsbModeForBand(string? band) => band switch
    {
        "160m" or "80m" or "40m" => RxMode.LSB,
        _ => RxMode.USB,
    };
