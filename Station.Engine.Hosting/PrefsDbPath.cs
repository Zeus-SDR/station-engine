...
// Central database-path resolver for the product zeus-prefs.db and the
// engine-owned station-engine.db. When ZEUS_PREFS_PATH is set, the engine DB
// follows it into the same directory unless ZEUS_ENGINE_PREFS_PATH explicitly
// overrides the engine file — useful for dev, CI, and isolated recovery.
//
...
// "Default" profile. Switching applies on the next launch (each path is
// resolved once at startup), so the endpoint that flips the pointer also asks
// the host to relaunch.
public static class PrefsDbPath
{
    public const string LegacyFileName = "zeus-prefs.db";
    public const string EngineFileName = "station-engine.db";
    public const string EnginePathEnvironmentVariable = "ZEUS_ENGINE_PREFS_PATH";
    private const string ProfileEngineFileSuffix = "-station-engine.db";
    ...
    /// <summary>
    /// Resolve the engine-owned settings database. By default it sits beside
    /// the active product preferences file, so ZEUS_PREFS_PATH continues to
    /// isolate both databases. The dedicated override is intentionally narrow:
    /// it moves only station-engine.db and never changes product preferences.
    /// </summary>
    public static string EngineGet()
    {
        var engineOverride = Environment.GetEnvironmentVariable(
            EnginePathEnvironmentVariable);
        if (!string.IsNullOrWhiteSpace(engineOverride))
            return engineOverride;

        return EnginePathForProductPath(Get());
    }

    public static string EnginePathForProductPath(string productDbPath)
    {
        var productFullPath = Path.GetFullPath(productDbPath);
        var directory = Path.GetDirectoryName(productFullPath) ?? DataDir;
        return Path.Combine(directory, EngineFileNameForProductPath(productFullPath));
    }

    internal static string EngineFileNameForProductPath(string productDbPath)
    {
        var productFileName = Path.GetFileName(productDbPath);
        if (productFileName.Equals(LegacyFileName, StringComparison.OrdinalIgnoreCase))
            return EngineFileName;

        var stem = Path.GetFileNameWithoutExtension(productFileName);
        if (string.IsNullOrWhiteSpace(stem))
            throw new ArgumentException(
                "Product preferences path must have a file name.",
                nameof(productDbPath));
        return stem + ProfileEngineFileSuffix;
    }

    internal static bool IsEngineDatabaseFileName(string fileName) =>
        fileName.Equals(EngineFileName, StringComparison.OrdinalIgnoreCase)
        || fileName.EndsWith(ProfileEngineFileSuffix, StringComparison.OrdinalIgnoreCase);
    ...
    //
    // Product preference stores share ONE physical zeus-prefs.db. If that file
    // is corrupt — most often an
    ...
    // file is guaranteed openable before any SharedLiteDatabase lease is taken.
    ...
    public static void SetActive(string relativePath)
    {
        ...
        if (IsEngineDatabaseFileName(Path.GetFileName(full)))
            throw new ArgumentException(
                "Engine databases cannot be selected as product preference profiles.",
                nameof(relativePath));
        if (!File.Exists(full))
    ...
    public static IReadOnlyList<PrefsDatabaseInfo> ListProfiles()
    {
        ...
        if (Directory.Exists(profilesDir))
        {
            ...
            {
                if (IsEngineDatabaseFileName(Path.GetFileName(file)))
                    continue;
    ...
    public static string CreateProfile(string name)
    {
        ...
        if (IsEngineDatabaseFileName(Path.GetFileName(target)))
            throw new InvalidOperationException(
                "Profile names ending in '-station-engine' are reserved for engine databases.");
    ...
    // Resolve a profile relative path to its validated full path under
    // DataDir. Throws when the path escapes the data dir or the file is
    // missing. Used by the merged profile export, which also needs the
    // profile's engine-side companion path (EnginePathForProductPath).
    internal static string ResolveProfileFullPath(string relativePath)
    {
        if (string.IsNullOrWhiteSpace(relativePath))
            throw new ArgumentException("Relative path is required.", nameof(relativePath));
