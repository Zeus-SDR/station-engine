...
// thin single-entry wrapper over RADE V1 (radae_c) + the Opus DNN/FARGAN
// vocoder that hides RADE's two-stage decode behind one streaming "complex IQ
// in -> 16 kHz PCM out" surface. Loaded dynamically as a shared library —
// zeus_rade.dll on
// Windows, libzeus_rade.so / libzeus_rade.dylib elsewhere — resolved by
// RadeNativeLoader (same once-per-assembly resolver as engine miniaudio).
//
// The native binary is built separately by CI and may be absent; every call
// site MUST guard via RadeNativeLoader.TryProbeRade() before invoking these.
//
...
internal static partial class RadeNativeMethods
{
    ...
    // TX-side mic tap meters, refreshed by the most recent zeus_rade_tx() call.
    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial int zeus_rade_tx_mic_level_db(IntPtr z);

    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial int zeus_rade_tx_mic_clip(IntPtr z);
    ...
    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial void zeus_rade_set_mic_agc_enabled(IntPtr z, int enable);

    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial void zeus_rade_set_mic_agc_target_db(IntPtr z, float target_dbfs);

    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial void zeus_rade_set_mic_eq_enabled(IntPtr z, int enable);

    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial void zeus_rade_set_mic_eq_bass_db(IntPtr z, float gain_db);

    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial void zeus_rade_set_mic_eq_mid_db(IntPtr z, float gain_db);

    [LibraryImport(LibraryName)]
    [UnmanagedCallConv(CallConvs = [typeof(CallConvCdecl)])]
    internal static partial void zeus_rade_set_mic_eq_treble_db(IntPtr z, float gain_db);
}
