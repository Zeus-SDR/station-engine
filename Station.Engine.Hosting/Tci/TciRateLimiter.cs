...
public sealed class TciRateLimiter : IDisposable
{
    ...
    public TciRateLimiter(int intervalMs, Action<string> send)
    {
        ...
        _send = send;
        // A period of 0 makes System.Threading.Timer fire ONCE and never
        // again, silently dropping every subsequent Enqueue. Interval <= 0
        // means "no rate limiting": skip the timer entirely and let Enqueue
        // send synchronously.
        _timer = intervalMs > 0
            ? new Timer(OnTick, null, intervalMs, intervalMs)
            : new Timer(OnTick, null, Timeout.Infinite, Timeout.Infinite);
    }
    ...
    public void Enqueue(string key, string commandLine)
    {
        if (_intervalMs <= 0)
        {
            try { _send(commandLine); }
            catch { /* match OnTick: a send failure must not throw into the caller */ }
            return;
        }
