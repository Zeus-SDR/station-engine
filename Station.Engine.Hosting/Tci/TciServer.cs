...
public sealed class TciServer : IHostedService, IDisposable
{
    ...
    public async Task AcceptAsync(HttpContext context)
    {
        ...
        try
        {
            ws = await context.WebSockets.AcceptWebSocketAsync(CreateWebSocketAcceptContext());
        }
        catch (Exception ex)
        {
    ...
    internal static WebSocketAcceptContext CreateWebSocketAcceptContext() => new()
    {
        // Naive TCI clients can drop on unsolicited control frames; #1401's
        // disconnect cadence matched the inherited 20 s WebSocket keepalive.
