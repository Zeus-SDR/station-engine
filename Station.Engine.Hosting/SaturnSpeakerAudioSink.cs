...
/// the two are mutually exclusive at
...
/// dedicated sender thread that drains the ring, packs PCM packets, and sends
/// through the connected Protocol-2 client. This keeps the DSP tick off the
/// network hot path so the host soundcard's playback ring is filled on time
/// (issue #1148). Cross-thread events from
...
internal sealed class SaturnSpeakerAudioSink : IRxAudioSink, IHostedService, IDisposable
{
    ...
    private static readonly long SendFailureBackoffTicks = Stopwatch.Frequency;
    ...
    // the next drain — set on fresh start, drain, disable, and transport loss).
    ...
    private long _sendRetryAfterTicks;
    ...
    private void OnSettingsChanged()
    {
        ...
        // pre-date the operator's last toggle.
    ...
    private void WorkerLoop()
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            if (!_wasEligible)
            {
                ...
                _log.LogInformation(
                    "audio.radio.speaker.p2 enabled endpoint={Endpoint} board={Board} variant={Variant} transport=shared",
                    state.Endpoint,
    ...
    private void DrainAndSend(Span<float> scratch)
    {
        ...
        {
            // The old dedicated socket retried at most once per second after a
        ...
        while (true)
        {
            ...
            if (!EmitPacket(now)) return; // transport disappeared mid-drain
            _nextSendTicks += PacketIntervalTicks;
        }
    }

    // Emit one full packet. Returns false when the active P2 transport went
    // away, so the drain loop stops. The test observer simulates a successful
    // send, advancing the sequence and recording the emit time.
    private bool EmitPacket(long now)
    {
        ...
        bool sent = SendPacket(now); // resets _packetFrames in its finally
        if (sent) RecordSend(now);
        return sent;
    }
    ...
    // DrainAndSend while the shared Protocol-2 transport is active.
    ...
    private bool IsEligible()
    {
        ...
        // RadioSpeakerAudioSink and self-gates on IsProtocol1Active. Speaker
        // packets reuse Protocol2Client's bound transport instead of creating a
        // second source port that can steal an Orion reply-route latch (#1457).
    ...
    private bool SendPacket(long now)
    {
        BinaryPrimitives.WriteUInt32BigEndian(_packet.AsSpan(0, sizeof(uint)), _sequence++);

        try
        {
            int sent = _radio.SendProtocol2SpeakerAudio(_packet);
            if (sent != PacketBytes)
            {
        ...
        catch (Exception ex) when (ex is SocketException or ObjectDisposedException)
        {
            ...
            _log.LogWarning(
                ex,
                "audio.radio.speaker.p2 send failed dropped={DroppedPackets}",
    ...
    private void BackOffSend(long now)
    {
        _sendRetryAfterTicks = now + SendFailureBackoffTicks;
        _nextSendTicks = 0;
        _pacingWaitTicks = SendFailureBackoffTicks;
    }
    ...
    // ---- Pacing test seams (#1148). No transport, no real time: drive a virtual
    ...
    /// <summary>Test-only: hard transport failure retry delay in Stopwatch ticks.</summary>
    internal static long SendFailureBackoffTicksForTest => SendFailureBackoffTicks;
    ...
    /// <see cref="RadioService"/> state assert against transport / sequence
