...
void SetRXARNNRRun (int channel, int run) {
  ...
  if (a->run != run) {
    RXAbp1CheckEx (channel, rxa[channel].amd.p->run, rxa[channel].snba.p->run,
...
RNNR create_rnnr(int run, int position, int size, double* in, double* out, int rate) {
  ...
  if (_rnnr_count == _rnnr_capacity) {
    int new_cap = _rnnr_capacity ? _rnnr_capacity * 2 :
                  4; // limit number of reallocs by doubling space each time, overkill but that is my middle name ;)
