...
DWORD LinuxWaitForMultipleObjects(DWORD count, HANDLE *handles, int wait_all, int ms) {
  if (count <= 0 || handles == NULL || wait_all) {
    return WAIT_FAILED;
  }

  int waited = 0;
  while (ms == INFINITE || waited < ms) {
    for (DWORD i = 0; i < count; i++) {
      if (sem_trywait((sem_t *)handles[i]) == 0) {
