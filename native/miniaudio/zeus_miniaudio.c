...
/* Resolve a serialized selection against the same context that will own the
 * capture device. CoreAudio device UIDs are context-owned backend data; feeding
 * a raw union reconstructed from an earlier enumeration into an implicit,
 * different context can open an active callback that contains only zeroes.
 * Keeping enumeration and ma_device_init on one context also canonicalizes the
 * ID for every backend instead of trusting padding bytes in ma_device_id. */
static ma_result zeus_ma_resolve_capture_device_id(
    ma_context* context,
    const char* requested_hex,
    ma_device_id* resolved_id)
{
    ...
    ma_device_id decoded_id;
    ma_uint32 i;

    if (context == NULL || requested_hex == NULL || resolved_id == NULL) {
        return MA_INVALID_ARGS;
    }
    if (!zeus_ma_decode_device_id(requested_hex, &decoded_id)) {
        return MA_INVALID_ARGS;
    }
    ...
    (void)playback_infos;
    (void)playback_count;
    for (i = 0; i < capture_count; i += 1) {
#if defined(MA_HAS_COREAUDIO)
        /* CoreAudio's stable identity is its NUL-terminated UID. Comparing
         * the whole 256-byte union also compares irrelevant tail bytes. */
        if (strcmp(capture_infos[i].id.coreaudio, decoded_id.coreaudio) == 0) {
#else
        if (ma_device_id_equal(&capture_infos[i].id, &decoded_id)) {
#endif
            memcpy(resolved_id, &capture_infos[i].id, sizeof(*resolved_id));
            return MA_SUCCESS;
        }
    }
    ...
    void* user)
{
    ...
    ma_device_id* requested_id_ptr = NULL;
    if (device_id_hex != NULL && device_id_hex[0] != '\0') {
        if (ma_context_init(NULL, 0, NULL, &h->context) != MA_SUCCESS) {
            free(h);
            return NULL;
        }
        h->context_initialized = MA_TRUE;
        if (zeus_ma_resolve_capture_device_id(
                &h->context,
                device_id_hex,
                &requested_id) != MA_SUCCESS) {
    ...
    if (ma_device_init(
            h->context_initialized ? &h->context : NULL,
            &cfg,
            &h->device) != MA_SUCCESS) {
        if (h->context_initialized) ma_context_uninit(&h->context);
        free(h);
        return NULL;
    }
